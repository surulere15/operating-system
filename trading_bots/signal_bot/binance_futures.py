...
class BinanceFuturesTrader:
    """Binance Futures trading with leverage"""
    ...
    def calculate_position_size(self,
        ...
        Args:
            signal: Trading signal from @AlphaEdgeSignals
            position_pct: Percentage of capital to use (default 30%)
        ...
        position_size_usd = position_capital * leverage  # With leverage
        ...
        # Risk check: Don't risk more than $3 per trade
    ...
    def place_stop_loss(self, symbol: str, position_info: Dict):
        ...
        try:
            ...
            stop_order = self.exchange.create_order(
                ...
                params={
                    'stopPrice': position_info['stop_loss']
                }
            )
    ...
    def place_take_profits(self, symbol: str, position_info: Dict):
        ...
        try:
            ...
            # Split position into 3 parts for TP levels
    ...
    def close_position(self, symbol: str, reason: str = "Manual close") -> Optional[Dict]:
        ...
        try:
            ...
            # Cancel remaining orders (stop loss, take profits)
    ...
    def monitor_positions(self):
        ...
        for symbol, pos in list(self.positions.items()):
            try:
                ...
                # Check if position hit stop loss or take profit (backup check)
                if pos['side'] == 'buy':
                    if current_price <= pos['stop_loss'] * 1.001:  # Allow 0.1% slippage
                        print(f"🛑 Stop loss triggered for {symbol}")
                        self.close_position(symbol, "Stop loss hit")
                    elif current_price >= pos['take_profits'][-1]:
                        print(f"🎯 Final take profit hit for {symbol}")
                        self.close_position(symbol, "Take profit hit")
                else:
                    if current_price >= pos['stop_loss'] * 0.999:
                        print(f"🛑 Stop loss triggered for {symbol}")
                        self.close_position(symbol, "Stop loss hit")
                    elif current_price <= pos['take_profits'][-1]:
                        print(f"🎯 Final take profit hit for {symbol}")
    ...
    def save_state(self, filepath: str = "/tmp/trading_state.json"):
...
def test_connection():
    ...
    print("\n🔧 Testing Binance Futures Connection...\n")
    ...
    trader = BinanceFuturesTrader(testnet=True, capital=100.0)
